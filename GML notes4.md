...
$$\begin{aligned} E_{\mathrm{in}}(w) &=\frac1N(Xw-y)^\top(Xw-y),\\ \nabla E_{\mathrm{in}}(w) &=\frac{2}{N}X^\top(Xw-y)\\ &=\frac{2}{N}(X^\top Xw-X^\top y). \end{aligned}$$
...
$$X^\top X \text{ is invertible} \iff X \text{ has linearly independent columns}.$$
...
$$\underset{\text{empirical training risk}}{E_{\mathrm{in}}(w)} \qquad\text{versus}\qquad \underset{\text{expected future risk}}{E_{\mathrm{out}}(w)}.$$

## Perceptron diagnostic

Let $`y_i\in\{-1,+1\}`$, and suppose the perceptron updates on a misclassified point using

$$w_{t+1} = w_t+\frac12\left(y_i-\mathrm{sign}(w_t^\top x_i)\right)x_i.$$
...
2. Show mathematically that the update improves the classification score of that particular example. Hint: compare
   ...
   $$y_iw_{t+1}^\top x_i \quad\text{with}\quad y_iw_t^\top x_i.$$
...
$$y_j w_{t+1}^{\top}x_j-y_jw_t^\top x_j = y_i y_j x_i^\top x_j.$$
...
$$y_i(w^{*\top}x_i)\ge \rho \qquad\text{for every }i,$$
...
$$R(\hat y\mid x) = \mathbb E[e(Y,\hat y)\mid x] = \sum_{y\in\mathcal Y}e(y,\hat y)p(y\mid x).$$
...
$$\begin{aligned} R(\hat y\mid x) &=\sum_y\mathbf 1[y\ne\hat y]p(y\mid x)\\ &=\sum_{y\ne\hat y}p(y\mid x)\\ &=1-p(\hat y\mid x). \end{aligned}$$

Consequently,

$$\arg\min_{\hat y}R(\hat y\mid x) = \arg\max_{\hat y}p(\hat y\mid x).$$
...
$$R(k+1)-R(k) = P(Y\le k)-P(Y\ge k+1) = 2F(k)-1.$$
...
$$\mathbb E[(Y-\hat y)^2] = \mathrm{Var}(Y) + \bigl(\mathbb E[Y]-\hat y\bigr)^2.$$
...
$$\begin{array}{c|c} \text{Loss} & \text{Bayes-optimal prediction}\\ 0/1 & \text{mode}\\ L_1 & \text{median}\\ \text{squared loss} & \text{mean} \end{array}$$
...
$$E_{\mathrm{in}}(h) = \frac1N\sum_{i=1}^N e(y_i,h(x_i))$$

is its observed training error. Its true generalization error is

$$E_{\mathrm{out}}(h) = \mathbb E_{(X,Y)\sim P}[e(Y,h(X))].$$
...
$$P\left( |E_{\mathrm{in}}(h)-E_{\mathrm{out}}(h)|>\epsilon \right) \le 2e^{-2N\epsilon^2}.$$
...
$$m_{\mathcal H}(N) = \max_{x_1,\ldots,x_N} \left| \left\{ (h(x_1),\ldots,h(x_N)):h\in\mathcal H \right\} \right|.$$
...
$$d_{\mathrm{VC}}(\mathcal H) = \max\{N:m_{\mathcal H}(N)=2^N\}.$$
...
$$m_{\mathcal H}(N) = \sum_{k=0}^{\min(3,N)}\binom Nk.$$

This gives

$$\begin{array}{c|ccccc} N&1&2&3&4&5\\ m_{\mathcal H}(N)&2&4&8&15&26 \end{array}$$
...
$$m_{\mathcal H}(5) = \binom50+\binom51+\binom52+\binom53 = 1+5+10+10=26.$$
...
$$E_{\mathrm{out}}(g) \le E_{\mathrm{in}}(g) + \sqrt{ \frac8N \log\left(\frac{4m_{\mathcal H}(2N)}{\delta}\right) }$$
...
$$\text{simple model} \Rightarrow \text{possibly underfits but generalizes more reliably}$$

$$\text{complex model} \Rightarrow \text{fits better but has greater overfitting risk}$$
...
$$g_D(x)-f(x) = \bigl(g_D(x)-\bar g(x)\bigr) + \bigl(\bar g(x)-f(x)\bigr).$$

Squaring gives

$$\begin{aligned} (g_D-f)^2 ={}&(g_D-\bar g)^2+(\bar g-f)^2\\ &+2(g_D-\bar g)(\bar g-f). \end{aligned}$$
...
$$\mathbb E_D[(g_D(x)-f(x))^2] = \underbrace{(\bar g(x)-f(x))^2}_{\text{bias}^2} + \underbrace{\mathbb E_D[(g_D(x)-\bar g(x))^2]}_{\text{variance}}$$

and, averaging over $`x`$,

$$\mathbb E_D[E_{\mathrm{out}}(g_D)] = \text{bias}^2+\text{variance}.$$

### Meaning

Bias measures whether the learning procedure is systematically wrong:

$$\mathrm{Bias}(x)=\bar g(x)-f(x).$$

Variance measures sensitivity to the particular training sample:

$$\mathrm{Var}(x) = \mathbb E_D[(g_D(x)-\bar g(x))^2].$$
...
$$Y=f(X)+\epsilon, \qquad \mathbb E[\epsilon\mid X]=0, \qquad \mathrm{Var}(\epsilon\mid X)=\sigma^2.$$

Then

$$\mathbb E_D[E_{\mathrm{out}}(g_D)] = \text{bias}^2+\text{variance}+\sigma^2.$$
...
$$E_{\mathrm{aug}}(w) = E_{\mathrm{in}}(w)+\lambda\Omega(w).$$
...
$$w_{\mathrm{reg}} = \arg\min_w \left(E_{\mathrm{in}}(w)+\lambda\|w\|_2^2\right).$$
...
$$\min_w E_{\mathrm{in}}(w) \quad\text{subject to}\quad \|w\|^2\le C.$$
...
$$\text{smaller }C \Longleftrightarrow \text{larger }\lambda \Longleftrightarrow \text{stronger regularization}.$$

## Past-exam regularization derivation

The exam gives

$$x_H^* = \arg\min_x(x-4)^2 \quad\text{subject to}\quad |x|\le C,$$

and

$$x_S^* = \arg\min_x\left((x-4)^2+\lambda x^2\right).$$
...
$$x_H^* = \begin{cases} C,&0<C<4,\\ 4,&C\ge4. \end{cases}$$
...
$$\frac{d}{dx}\left((x-4)^2+\lambda x^2\right) = 2(x-4)+2\lambda x.$$
...
$$\lambda= \begin{cases} \frac4C-1,&0<C<4,\\ 0,&C\ge4. \end{cases}$$
...
$$E_{\mathrm{test}}(g^-) = \frac1K \sum_{(x_i,y_i)\in D_{\mathrm{test}}} e(y_i,g^-(x_i)).$$

Because the test set did not influence $`g^-`$,

$$\mathbb E[E_{\mathrm{test}}(g^-)\mid g^-] = E_{\mathrm{out}}(g^-).$$
...
$$\text{Repeating splits reduces variance, not training-size bias.}$$
...
$$E_{\mathrm{CV}} = \frac1L\sum_{l=1}^L E_{\mathrm{test}}(g_l^-).$$
...
$$\text{Regularization controls complexity;} \quad \text{validation selects complexity;} \quad \text{testing estimates final performance.}$$
...
$$\Phi(x)= \begin{pmatrix} 1\\x_1\\x_2\\x_1^2\\x_2^2\\x_1x_2 \end{pmatrix}.$$

The resulting model is

$$h_w(x) = w_0+w_1x_1+w_2x_2+w_3x_1^2+w_4x_2^2+w_5x_1x_2.$$
...
$$\text{richer }\Phi \quad\Rightarrow\quad \text{lower potential training error} \quad\text{but greater overfitting risk}.$$
...
$$\frac{\partial e}{\partial w_{ij}^{(l)}} = x_i^{(l-1)}\delta_j^{(l)}.$$

This is the key gradient formula:

$$\text{gradient} = \text{input activation} \times \text{downstream error signal}.$$

### Output-layer error

At the output layer,

$$\delta_j^{(L)} = \frac{\partial e}{\partial x_j^{(L)}} \theta'\left(s_j^{(L)}\right).$$
...
$$\begin{aligned} \delta_i^{(l-1)} &= \frac{\partial e}{\partial s_i^{(l-1)}}\\ &= \theta'(s_i^{(l-1)}) \sum_{j=1}^{d^{(l)}} w_{ij}^{(l)}\delta_j^{(l)}. \end{aligned}$$

Thus,

$$\delta_i^{(l-1)} = \theta'(s_i^{(l-1)}) \sum_jw_{ij}^{(l)}\delta_j^{(l)}.$$
...
$$\delta_i^{(l-1)} = \left(1-(x_i^{(l-1)})^2\right) \sum_jw_{ij}^{(l)}\delta_j^{(l)}.$$
...
$$e(y,\hat p) = -y\log\hat p-(1-y)\log(1-\hat p).$$

An important simplification occurs:

$$\frac{\partial e}{\partial s} = \hat p-y.$$

To see this, use

$$\frac{\partial e}{\partial\hat p} = -\frac{y}{\hat p} + \frac{1-y}{1-\hat p}$$

and

$$\frac{\partial\hat p}{\partial s} = \hat p(1-\hat p).$$

Multiplying and simplifying gives $`\hat p-y`$.

This pairing is mathematically convenient and gives a useful interpretation:

$$\text{gradient signal} = \text{predicted probability} - \text{true label}.$$

### Logistic regression

Logistic regression assumes

$$\log\left(\frac{p(x)}{1-p(x)}\right) = w^\top x.$$
...
$$e(y,\hat p) = -\sum_{k=1}^K y_k\log\hat p_k.$$
...
$$\frac{\partial e}{\partial s_k} = \hat p_k-y_k.$$
...
$$X_i<t \qquad\text{versus}\qquad X_i\ge t.$$
...
$$\mathrm{Gain} = H(p) - \sum_{j=1}^m\frac{n_j}{n}H(p^{(j)}).$$
...
$$\mathrm{Gain} = V_{\text{parent}} - \sum_j\frac{|D_j|}{|D|}V_j.$$
...
$$H(\text{parent}) - \sum_j\frac{n_j}{n}H(\text{child}_j).$$
...
$$h_k(x): \quad y_k \text{ versus } \mathcal Y\setminus\{y_k\}.$$

For model $`k`$, transform the labels into

$$c_n^{(k)} = \begin{cases} +1,&c_n=y_k,\\ -1,&c_n\ne y_k. \end{cases}$$
...
$$M= \begin{pmatrix} +1&-1&\cdots&-1\\ -1&+1&\cdots&-1\\ \vdots&\vdots&\ddots&\vdots\\ -1&-1&\cdots&+1 \end{pmatrix}.$$
...
$$d_H(v,m_j) \ge d_H(m_k,m_j)-d_H(v,m_k) \ge d_{\min}-t.$$
...
$$\begin{array}{c|rrrrrr} &h_1&h_2&h_3&h_4&h_5&h_6\\ y_1&+&+&+&-&-&-\\ y_2&+&-&-&+&+&-\\ y_3&-&+&-&+&-&+\\ y_4&-&-&+&-&+&+ \end{array}$$
...
$$\begin{aligned} d_H(v,m_1)&=3,\\ d_H(v,m_2)&=5,\\ d_H(v,m_3)&=1,\\ d_H(v,m_4)&=3. \end{aligned}$$
...
$$P(A\mid x) = P(\{A,D\}\mid x) P(A\mid x,\{A,D\}).$$
...
$$h(x)=\mathrm{AGG}(h_1(x),\ldots,h_M(x)).$$
...
$$\mathrm{Var}\left(\frac1M\sum_{m=1}^Mh_m\right) = \sigma^2\left( \rho+\frac{1-\rho}{M} \right).$$

As $`M\to\infty`$,

$$\mathrm{Var}\rightarrow\rho\sigma^2.$$
...
$$\epsilon_k = \sum_{n=1}^N w_n^{(k-1)} \mathbf1[h_k(x_n)\ne y_n].$$
...
$$w_n^{(k)} = \frac1Z w_n^{(k-1)} \exp\left(-\alpha_k y_nh_k(x_n)\right).$$
...
$$h(x) = \mathrm{sign} \left( \sum_k\alpha_kh_k(x) \right).$$
...
$$\sum_{v=1}^m\sum_{i=1}^L e(y_i,h_v(x_i)) + \lambda_1\sum_v\|h_v\|^2 + \lambda_2 \sum_{u,v} \sum_{j=L+1}^N \bigl(h_u(x_j)-h_v(x_j)\bigr)^2.$$
...
$$P(y\mid x) = \frac{P(x,y)}{\sum_{\bar y}P(x,\bar y)}.$$
...
$$P(x_j\mid\theta) = \sum_yP(x_j,y\mid\theta).$$

Hence,

$$L(\theta) = \prod_{i=1}^L P(x_i,y_i\mid\theta) \prod_{j=L+1}^N \sum_yP(x_j,y\mid\theta).$$
...
$$J(y) = \sum_{1\le i<j\le N} w_{ij}|y_i-y_j|$$

subject to the labels of the labeled observations remaining fixed.

Because $`y_i,y_j\in\{0,1\}`$,

$$|y_i-y_j| = \begin{cases} 0,&y_i=y_j,\\ 1,&y_i\ne y_j. \end{cases}$$

Therefore, edge $`(i,j)`$ contributes:

$$\begin{cases} 0,&\text{same label},\\ w_{ij},&\text{different labels}. \end{cases}$$
...
$$1\le1.5, \qquad \sqrt2\le1.5, \qquad 2>1.5.$$
...
$$\begin{aligned} (1,3)&\leftrightarrow(2,4),\\ (2,4)&\leftrightarrow(3,3),\\ (4,4)&\leftrightarrow(3,3),\\ (3,3)&\leftrightarrow(4,2),\\ (4,2)&\leftrightarrow(3,1),\\ (3,1)&\leftrightarrow(2,1). \end{aligned}$$
...
$$\begin{aligned} (1,4)&\leftrightarrow(2,4),(2,3),(2,5),\\ (2,4)&\leftrightarrow(1,4),(2,3),(3,3),(2,5),\\ (4,4)&\leftrightarrow(3,3),\\ (4,2)&\leftrightarrow(3,1),(5,2),(3,3),\\ (2,3)&\leftrightarrow(3,3). \end{aligned}$$

The leaf-like nodes are immediate:

$$\begin{aligned} (3,1)&:\text{positive},\\ (5,2)&:\text{positive},\\ (2,5)&:\text{negative}. \end{aligned}$$
...
$$\begin{aligned} (3,1)&:+,\\ (5,2)&:+,\\ (2,3)&:-,\\ (2,5)&:-,\\ (3,3)&:\text{either }+\text{ or }-. \end{aligned}$$
...
$$P(S_{t+1}=s'\mid S_t,A_t,\text{past}) = P(S_{t+1}=s'\mid S_t,A_t).$$
...
$$\begin{aligned} V^\pi(s) &= \mathbb E_\pi[G_t\mid S_t=s]\\ &= \mathbb E_\pi[r_{t+1}+\gamma G_{t+1}\mid S_t=s]\\ &= \sum_a\pi(a\mid s) \sum_{s'}T(s,a,s') \left[ r(s,a,s')+\gamma V^\pi(s') \right]. \end{aligned}$$
...
$$\text{current value} = \text{immediate reward} + \text{discounted expected next value}.$$
...
$$V^*(s) = \max_{a\in\mathcal A(s)} \sum_{s'}T(s,a,s') \left[ r(s,a,s')+\gamma V^*(s') \right].$$
...
$$\pi^*(s) \in \arg\max_a \left[ r(s,a) + \gamma\sum_{s'}T(s,a,s')V(s') \right].$$

### Why does it converge?

Define the Bellman optimality operator

$$(\mathcal TV)(s) = \max_a \left[ r(s,a)+\gamma\sum_{s'}T(s,a,s')V(s') \right].$$

It is a contraction under the maximum norm:

$$\|\mathcal TV-\mathcal TU\|_\infty \le \gamma\|V-U\|_\infty.$$
...
$$V^{\pi_k}(s) = r(s,\pi_k(s)) + \gamma\sum_{s'}T(s,\pi_k(s),s')V^{\pi_k}(s').$$
...
$$\pi_{k+1}(s) \in \arg\max_a \left[ r(s,a) + \gamma\sum_{s'}T(s,a,s')V^{\pi_k}(s') \right].$$
...
$$T(s,a,s') \quad\text{and}\quad r(s,a,s').$$
...
$$\begin{aligned} \text{SARSA target} &= r+\gamma Q(s',a'_{\text{actually selected}}),\\ \text{Q-learning target} &= r+\gamma\max_{a'}Q(s',a'). \end{aligned}$$

### Small numerical example

Suppose

$$Q(s,a)=2,\quad r=1,\quad \gamma=0.9,\quad \alpha=0.1.$$
...
$$A_t= \begin{cases} \arg\max_aQ(S_t,a),&\text{with probability }1-\epsilon,\\ \text{random action},&\text{with probability }\epsilon. \end{cases}$$
...
$$\begin{array}{c|c|c} \text{Method}&\text{Model required?}&\text{Target}\\ \text{Value iteration}&\text{yes}&\text{Bellman optimum}\\ \text{Policy iteration}&\text{yes}&\text{evaluate, then improve}\\ \text{Monte Carlo}&\text{no}&\text{complete observed return}\\ \text{TD}&\text{no}&r+\gamma V(s')\\ \text{SARSA}&\text{no}&r+\gamma Q(s',a')\\ \text{Q-learning}&\text{no}&r+\gamma\max_{a'}Q(s',a') \end{array}$$
...
$$Q(s',a') \quad\text{and}\quad \max_{a'}Q(s',a'),$$
